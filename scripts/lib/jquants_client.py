...
- **`/equities/bars/daily` のクエリ方式（レンジ取得が可能か）は未確認**であるため、
  ...
  返るか」を検出し、**どちらも確認できない場合は憶測で全件走査に入らず停止する**
  設計にしている（EXP-OBS000037 の教訓＝算術的な矛盾や未確認の前提を自己解釈で
  埋めない、をAPI呼び出し設計にも適用したもの）。**全件走査を強制するCLIフラグは
  意図的に実装していない**（8万件超×5秒≈4.6日という実行時間になり、それを
  安易なフラグ1つで踏ませるのは危険なため。停止した場合はJ-Quants API
  リファレンスでの正式なクエリ方式の確認、またはS戦略チームへの相談が必要）。
...
def detect_bars_daily_strategy(
    ...
    *,
    ...
    log.append(
        ...
        "本スクリプトはここで停止する（全件走査を強制するCLIフラグは意図的に"
        "実装していない）。件数を絞った代替案（例: 月内複数日サンプリング）を"
        "検討する場合は、それ自体をS戦略チームへの差し戻し事項として相談すること。"
